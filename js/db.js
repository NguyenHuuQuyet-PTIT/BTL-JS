// ==========================================
// 1. CƠ SỞ DỮ LIỆU & HÀM TIỆN ÍCH
// ==========================================
window.getDB = key => JSON.parse(localStorage.getItem(key)) || [];
window.setDB = (key, data) => localStorage.setItem(key, JSON.stringify(data));

window.updateClassDB = (classId, updateFunction) => {
    let classes = getDB('Classes');
    let target = classes.find(c => c.id === classId);
    if (target) { updateFunction(target, classes); setDB('Classes', classes); }
};
...
window.getPeriodText = (start, end) => `Tiết ${start}-${end} (${PERIOD_TIMES[start].split("-")[0]} - ${PERIOD_TIMES[end].split("-")[1]})`;

// ==========================================
// 2. LOGIC ĐIỀU HƯỚNG DÙNG CHUNG (MENU & TABS)
// ==========================================
window.switchSubTab = (btnId, tabId, btnClassGroup, tabClassGroup) => {
...
function initDB() {
    ...
    if (!localStorage.getItem('Subjects')) {
        setDB('Subjects', [ { id: 'SUB01', name: 'Lập trình Web', abbr: 'WEB' }, { id: 'SUB02', name: 'Cấu trúc dữ liệu', abbr: 'CTDL' }, { id: 'SUB03', name: 'Cơ sở dữ liệu', abbr: 'CSDL' } ]);
    }
    if (!localStorage.getItem('Classes')) {
        setDB('Classes', [
            { 
                id: 'WEB_L1', subjectId: 'SUB01', teacherId: 'GV001', room: 'A101', dayOfWeek: 'Thứ 2', 
                startDate: '2026-06-01', endDate: '2026-07-31', startPeriod: 1, endPeriod: 3, enrolledStudents: ['SV202501'], 
                sessions: [ { id: 'S1', date: '2026-06-01', startPeriod: 1, endPeriod: 3, attendance: {'SV202501': 'present'} } ], grades: { 'SV202501': { cc: 10, gk: 8, ck: 9 } } 
            }
        ]);
    }
}

...
* **Đặc điểm nổi bật:** Tự động bắt lỗi tràn bộ nhớ (giới hạn 5MB của trình duyệt). Nếu phát hiện bộ nhớ đầy, hàm sẽ tự động xóa bớt các file đính kèm cũ để ứng dụng luôn chạy ổn định. Đặc biệt, bộ đôi này tự động mã hóa dữ liệu nhạy cảm dưới dạng XOR & Base64 trước khi lưu xuống bộ nhớ máy khách, ngăn chặn hoàn toàn nguy cơ rò rỉ dữ liệu cá nhân (như tài khoản, token) khi bị tấn công XSS hoặc đọc lén LocalStorage.
...
### 5. Hàm mở modal xem chi tiết bài nộp học viên (`moModalChiTietBaiNop` - nằm trong `app.js`)
* **Vai trò:** Tự động mở xem chi tiết bài làm nộp trực tuyến của sinh viên khi giáo viên nhấp vào thông báo.
* **Cách hoạt động:** Nhận dạng tệp đính kèm bài nộp và kết xuất xem thử inline trực quan trực tiếp ngay trên trang (PDF, Word, Ảnh, Video, v.v.), không bắt buộc tải file gốc về máy. Hỗ trợ cơ chế tự động tải on-demand từ server nếu tệp đã bị xóa khỏi LocalStorage do đầy bộ nhớ cache.

---

## 5. Nâng cấp hiệu năng & Khắc phục lỗi giới hạn tệp đính kèm
- **Nâng cấp dung lượng Request Backend:** Tăng giới hạn kích thước nhận dữ liệu JSON và URL-encoded trên server Node/Express lên `100mb` (sử dụng cấu hình `express.json({ limit: '100mb' })`). Điều này giúp khắc phục triệt để lỗi `413 Payload Too Large` khi giáo viên đăng tài liệu PDF/Word lớn hoặc khi sinh viên nộp bài làm có đính kèm tệp dung lượng cao.
- **Tính năng Thông báo nộp bài tự động chi tiết:** Khi học viên nộp hoặc cập nhật bài làm, hệ thống tự động sinh thông báo gửi thẳng đến hòm thư của Giảng viên phụ trách lớp học phần đó, cung cấp đầy đủ thông tin: Họ tên sinh viên, Mã số sinh viên, Lớp học phần, Tiêu đề bài tập, Ngày nộp và Tên tệp. Khi giảng viên nhấp vào thông báo này, giao diện lập tức mở bản xem trước inline chi tiết mà không cần thông qua các bước tìm kiếm thủ công.